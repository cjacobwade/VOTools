...
public static class WavUtility
{
	...
	public static AudioClip TrimSilence(this AudioClip clip)
	{
		int i;
		float min = 0f;
		...
		{
			if (Mathf.Abs(samplesList[i]) > min)
				break;
		}

		samplesList.RemoveRange(0, i);
		...
		AudioClip trimmedClip = AudioClip.Create(clip.name, samplesList.Count, clip.channels, clip.frequency, false);
		trimmedClip.SetData(samplesList.ToArray(), 0);
